...
class D693_ZG_LightDevice extends HzcZigBeeLightDevice {
    ...
    async changeDimLevel2(dim, opts = {}) {
        ...
        return this.levelControlCluster.moveToLevelWithOnOff(moveToLevelWithOnOffCommand)
            ...
            this.log('---------changeDimLevel2-4', new Date())
        return
 
        try {
            this.log('---------changeDimLevel2-1', new Date())
            await this.levelControlCluster2().moveToLevelWithOnOff(moveToLevelWithOnOffCommand)
                .then(async result => { 
                    //await wait(5000).then(async () => {
                        if (dim === 0) {
                            await this.setCapabilityValue('onoff', false);
                            await this.setCapabilityValue('dim', 0)
                            this.log('-----set dim = 0, off')
                        } else if (this.getCapabilityValue('onoff') === false && dim > 0) {
                            this.log('-----set dim = ', dim, 'on')
                            await this.setCapabilityValue('onoff', true);
                        }
                        this.log('---------changeDimLevel2-2', new Date())
                    //})  
            ...
            this.log('--------changeDimLevel() error: ', error) 
            return ""+error
        } 
        this.log('---------changeDimLevel2-4', new Date())
        return true;
...
module.exports = D693_ZG_LightDevice

...
/**
 * The HzcZigBeeLightDevice class handles all light related capabilities [`onoff`, `dim`,
 ...
 *
 * @example
 * const { HzcZigBeeLightDevice } = require('homey-zigbeedriver');
 *
 * class ZigBeeBulb extends HzcZigBeeLightDevice {
 ...
 * }
 */
class HzcZigBeeLightDevice extends ZigBeeDevice {
  ...
  async onNodeInit({ zclNode, supportsHueAndSaturation, supportsColorTemperature }) {
    ...
    this.setStoreValue('colorTempMin', 153); // 1000000/6500K = 153 Mired
    this.setStoreValue('colorTempMax', 500); // 1000000/2000K = 500 Mired
    ...
    this.log('HzcZigBeeLightDevice is initialized', {
  ...
  async changeDimLevel(dim, opts = {}) {
    ...
    const moveToLevelWithOnOffCommand = {
      ...
      transitionTime: /*calculateLevelControlTransitionTime(opts)*/0,
    };
  ...
  async changeColorTemperature(temperature, opts = {}) {
    ...
    if (this.supportsColorTemperature) {
      ...
      this.log("===========================>colorTemperature = "+colorTemperature);
      // Execute move to color temperature command
      const moveToColorTemperatureCommand = {
        colorTemperature,
        transitionTime: /*calculateColorControlTransitionTime(opts)*/0,
      };
  ...
  async changeColor({ hue, saturation, value }, opts = {}) {
    ...
    this.log('changeColor() → his.hasCapability(light_mode) = ', this.hasCapability('light_mode'), ",his.getCapabilityValue(light_mode)", this.getCapabilityValue('light_mode'));
    ...
    this.log('changeColor() → convertHSVToCIE ', { hue, saturation, value });
  ...
  async onEndDeviceAnnounce() {
    ...
    if (this.supportsColorTemperature && typeof colorTemperatureMireds === 'number') {
      ...
      this.log("setCapabilityValue light_temperature = " + temp_value);

      await this.setCapabilityValue('light_temperature', temp_value);
    }
  }

}

module.exports = HzcZigBeeLightDevice;

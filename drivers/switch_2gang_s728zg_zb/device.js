...
class S728zg_2gang_switch_Device extends HzcSwitch2GangZigBeeDevice {
    ...
    async onNodeInit({ zclNode }) {
        ...
        const HzcBasicCluster = require('../../lib/HzcBasicCluster')
        Cluster.addCluster(HzcBasicCluster)
        this.log('+Cluster add HzcBasicCluster: ', this.zclNode.endpoints[1].clusters[CLUSTER.BASIC.NAME])

        this.enableDebug();
        this.printNode();   

        if (this.hasCapability('button.calibrate')) { await this.removeCapability('button.calibrate'); } 
    ...
    async initApp() {
        ...
        if (this.curMode === 0 || this.curMode === '0') {
            ...
            if (this.hasCapability('rms_voltage')) { await this.removeCapability('rms_voltage'); }
            if (this.hasCapability('rms_current')) { await this.removeCapability('rms_current'); }
            ...
            if (this.hasCapability('switch_1')) { await this.removeCapability('switch_1'); }
            if (this.hasCapability('switch_2')) { await this.removeCapability('switch_2'); }
            if (this.hasCapability('meter_power_1')) { await this.removeCapability('meter_power_1'); }
            if (this.hasCapability('measure_power_1')) { await this.removeCapability('measure_power_1'); }
            if (this.hasCapability('meter_power_2')) { await this.removeCapability('meter_power_2'); }
            if (this.hasCapability('measure_power_2')) { await this.removeCapability('measure_power_2'); }

            if (this.hasCapability('rms_voltage_1')) { await this.removeCapability('rms_voltage_1'); }
            if (this.hasCapability('rms_current_1')) { await this.removeCapability('rms_current_1'); }
            if (this.hasCapability('rms_voltage_2')) { await this.removeCapability('rms_voltage_2'); }
            if (this.hasCapability('rms_current_2')) { await this.removeCapability('rms_current_2'); }
